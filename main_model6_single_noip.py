...
def trans_embed(model, sample_tensor, labels):
    ...
    seq_tensor = torch.zeros((len(sample_tensor), sample_length.max(), 12))
...
def train(**kwargs):
    ...
    model_vec = [models.model_4(41, 16, 2, 4, opt.use_gpu) for i in range(30)]
    """
    ...
    for i in range(30):
        model_vec[i] = train_single(model_vec[i], training_set, i, 3000)

    validate(model_vec, val_data)
...
def validate(model_vec, flowextractor):
    opt = DefaultConfig()
    ...
    validate_set = gen_mixture(windows, labels, opt.capacity, 2)
