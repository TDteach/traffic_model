...
def train(**kwargs):
    ...
    model = models.CNN_LSTM(71, 64, 320, 30, 4)
    ...
    train_data = Dispatcher(opt.csv_paths, opt.used_size, opt.capacity, opt.window_size*5, opt.step, filter=opt.mask, timeinterval=True)
    val_data = Dispatcher(opt.csv_paths, opt.used_size, opt.capacity, opt.window_size*5, opt.step, test=True, filter=opt.mask, timeinterval=True)
    ...
    #train_data = FlowExtractor(opt.csv_paths, opt.used_size, opt.capacity, opt.window_size, opt.step, filter=opt.mask, timeinterval=True)
    #val_data = FlowExtractor(opt.csv_paths, opt.used_size, opt.capacity , opt.window_size, opt.step, test=True, filter=opt.mask, timeinterval=True)
    train_dataloader = DataLoader(train_data,batch_size=1,shuffle=True,drop_last=True, num_workers=opt.num_workers)#,num_workers=opt.num_workers)
    val_dataloader = DataLoader(val_data,batch_size=1,shuffle=False,num_workers=opt.num_workers,drop_last=True)
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            start_time = sample_tensor[0][0, -1]
            for j in range(len(sample_tensor[0])):
                prev = sample_tensor[0][j, -1]
                sample_tensor[0][j, -1] -= start_time
                start_time = prev

            batch_size, window_size, feature_len = sample_tensor.size() #(1, 500, 13)
            ...
            var_embed = var_embed.view(5, window_size/5, 71)
            ...
            #print 'last_output.size()', last_output.size(), 'label_tensor.size()', label_tensor.size()
...
def validate(model, dataloader):
    ...
    for i, data in enumerate(dataloader):
        ...
        start_time = val_input[0][0, -1]
        for j in range(len(val_input[0])):
            prev = val_input[0][j, -1]
            val_input[0][j, -1] -= start_time
            start_time = prev
