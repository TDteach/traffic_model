...
def train(**kwargs):
    ...
    val_data = FlowExtractor_time(opt.csv_paths, opt.used_size, opt.capacity/2 , opt.window_size, opt.step, test=True, filter=opt.mask)
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            """
            sample_tensor =batch['window'] #batch['window'].type(torch.cuda.FloatTensor)
            print 'sample_tensor.size()', sample_tensor.size()
            """
