...
"""
['port','ipv4','ipv6','tcp','udp','http','ssl','dns','direction1','direction2','datalen','timeinterval', 'tag']
"""
def train(**kwargs):
    opt = DefaultConfig()
    opt.parse(kwargs)
    ...
    #model = models.CNN(11)
    model = models.CNN_noip_noport_noprot(11)
    ...
    val_data = Dispatcher(opt.csv_paths, opt.used_size, opt.capacity, opt.window_size, opt.step, test=True, filter=['ip', 'port','ipv4','ipv6','tcp','udp','http','ssl','dns','direction1','direction2','datalen','timeinterval','tag'], timeinterval=True)
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            var_embed = var_embed.view(batch_size, -1, window_size, 11)
            if opt.use_gpu:
