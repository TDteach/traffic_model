...
"""
In this version, we make gen_mix procedure consistent.
which means svm/cnn/rnn will receive same mixed traffic instead of generating their own versions.
"""
fea_len = 71
...
def gen_validate(X_test, y_test, num, mix, algorithm='nn'):
    ...
    if mix==1:
        ...
        if algorithm=='svm':
            ...
            new_X = X
        validate_set = zip(new_X, y)
    else:   
        validate_set = gen_hardmix(X, y, num, mix, algorithm, ip=True, port=True)
    return validate_set
...
def val_svm(model, scaler, validate_set, mix=1, level=1):
    #Data Preprocessing
    assert mix>0
    X_test, y_test = zip(*validate_set)
...
def wrapper_svm(model, X_train, y_train, validate_sets):
    model, scaler = train_svm(model, X_train, y_train)
    val_svm(model, scaler, validate_sets[0], 1)
    val_svm(model, scaler, validate_sets[1], 2)
    val_svm(model, scaler, validate_sets[2], 4)
...
def val_CNN(model, validate_set, mix=1):
    assert mix>0
...
def wrapper_CNN(model, X_train, y_train, validate_sets):
    train_CNN(model, X_train, y_train)
    val_CNN(model, validate_sets[0], 1)
    val_CNN(model, validate_sets[1], 2)
    val_CNN(model, validate_sets[2], 4)
...
def val_LSTM(model, validate_set, mix=1):
    assert mix>0
    opt = DefaultConfig()
    correct, false_p, false_n, total = 0, 0, 0, 0
    model.eval()
...
def wrapper_LSTM(model, X_train, y_train, validate_sets):
    train_LSTM(model, X_train, y_train)
    val_LSTM(model, validate_sets[0], 1)
    val_LSTM(model, validate_sets[1], 2)
    val_LSTM(model, validate_sets[2], 4)
...
if __name__ == "__main__":
    ...
    validate_sets_svm = []
    validate_sets_svm.append(gen_validate(X_test, y_test, 4000, 1, 'svm'))
    validate_sets_svm.append(gen_validate(X_test, y_test, 4000, 2, 'svm'))
    validate_sets_svm.append(gen_validate(X_test, y_test, 4000, 4, 'svm'))

    validate_sets_nn = []
    validate_sets_nn.append(gen_validate(X_test, y_test, 4000, 1, 'nn'))
    validate_sets_nn.append(gen_validate(X_test, y_test, 4000, 2, 'nn'))
    validate_sets_nn.append(gen_validate(X_test, y_test, 4000, 4, 'nn'))
    ...
    p1 = Process(target=wrapper_svm, args=(model_svm, X_train, y_train,validate_sets_svm))
    p1.start()
    #wrapper_CNN(model_cnn, X_train, y_train, validate_sets_nn)
    #wrapper_LSTM(model_lstm, X_train, y_train, validate_sets_nn)
    #wrapper_CNN(model_cnnlstm, X_train, y_train, validate_sets_nn)
    p1.join()
