...
def train_svm(model, X_train, y_train):
    ...
    new_X = [genStatisVec3(x, False, True) for x in X]
    #normalization
...
def val_svm(model, scaler, X_test, y_test, mix=1, level=1):
    #Data Preprocessing
    assert mix>0
...
def wrapper_svm(model, X_train, y_train, X_test, y_test):
    ...
    val_svm(model, scaler, X_test, y_test, 1)
    val_svm(model, scaler, X_test, y_test, 2, 2)
    val_svm(model, scaler, X_test, y_test, 4, 2)
...
def train_LSTM(model, X_train, y_train, load_path = '/home/4tshare/iot/dev/traffic_model/checkpoints'):
    ...
    if opt.use_gpu:
        ...
        loss_function = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=opt.lr)
    if not os.path.exists(os.path.join(load_path, model.name + '_noip')):
        for epoch in range(opt.max_epoch):
            correct, total = 0, 0
            print 'epoch', epoch
            for i, batch in enumerate(trDataloader):
            ...
            print 'epoch ', epoch, 'accuracy ', 1.0*correct/total
        model.cpu()
        torch.save(model.state_dict(), os.path.join(load_path, model.name+'_noip'))
        print 'model saved successfully.'
    else:
        model.load_state_dict(torch.load(os.path.join(load_path, model.name+'_noip')))
    #Do simple validation on "training set"
    if opt.use_gpu:
        model.cuda()
...
if __name__ == "__main__":
    ...
    #model_cnn = CNN(71, 30, 60)
    #model_lstm = LSTM(71, 16, 32, 30, 2, 60)
    #model_liLSTM = LinearLSTM(71, 128, 16, 32, 30, 2, 60)
    #model_cnnlstm = CNN_LSTM2(71, 64, 128, 16, 30, 2, 60)
