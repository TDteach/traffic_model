...
class model_2(BasicModule):
    ...
    def __init__(self, feature_dim, hidden_dim, tagset_size, num_layers, cuda_support=True):
        ...
        self.ipembed = nn.Embedding(len(ipdict)+1,10)#, padding_idx=0)
        self.portembed = nn.Embedding(len(portdict)+1,10)#, padding_idx=0)
        self.protembed = nn.Embedding(len(protdict)+1,10)#, padding_idx=0)
        """
        nn.Embedding can only handle autograd.Variable
        """
        self.lstm = nn.LSTM(feature_dim, hidden_dim, num_layers, dropout=0.5, bidirectional=True)
    ...
    def forward(self, var_embed):
        ...
        :return:
        """
        #h0 = Variable(torch.nn.init.orthogonal(torch.Tensor(self.num_layers*2, 1, self.hidden_dim)).cuda())
        #c0 = Variable(torch.nn.init.orthogonal(torch.Tensor(self.num_layers*2, 1, self.hidden_dim)).cuda())
        h0 = Variable(torch.zeros(self.num_layers*2, 1, self.hidden_dim).cuda())
        ...
        if self.cuda_support:
            ...
            tag_scores = F.log_softmax(tag_space[-1,-1,:])
            return tag_scores
